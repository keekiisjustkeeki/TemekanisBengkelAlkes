...
// Load contact information from localStorage (admin controlled)
function loadContactInfo() {
    const kontak = JSON.parse(localStorage.getItem('temeknis_kontak') || '{}');
    
    // Default values if not set by admin
    const defaultKontak = {
        alamat: 'Jl. Biduri Senteluk Daye, Desa Senteluk, Batulayar, Lombok Barat, NTB',
        telepon: '081991303343',
        email: 'temmekanis@gmail.com',
        whatsapp: '6281991303343'
    };
    
    // Merge with defaults
    const data = { ...defaultKontak, ...kontak };
    
    // Update contact section if exists
    const alamatEl = document.getElementById('kontak-alamat');
    const teleponEl = document.getElementById('kontak-telepon');
    const emailEl = document.getElementById('kontak-email');
    const waEl = document.getElementById('kontak-wa');
    const waLinkEl = document.getElementById('kontak-wa-link');
    ...
    if (waLinkEl) waLinkEl.href = 'https://wa.me/' + data.whatsapp + '?text=Halo%20TeMekanis';
}

// Load gallery images from localStorage (admin controlled)
function loadGalleryImages() {
    ...
    // Default images if admin hasn't set any
    ...
    // Get visible gallery indices from localStorage
    let visibleGallery = JSON.parse(localStorage.getItem('temeknis_galeri_visible'));
    
    // If no visible list exists, create one with all indices and save it
    if (!visibleGallery || visibleGallery.length === 0) {
        visibleGallery = galleryImages.map((_, index) => index);
        localStorage.setItem('temeknis_galeri_visible', JSON.stringify(visibleGallery));
    }
    
    // Filter gallery based on visibility - only show images at visible indices
    let filteredImages = galleryImages.filter((img, index) => visibleGallery.includes(index));
    
    // If no visible images after filtering, show all
    ...
    filteredImages.forEach((imageSrc, index) => {
        const card = document.createElement('div');
        card.className = 'gallery-card';
        card.innerHTML = `<img src="${imageSrc}" alt="Galeri Perbaikan ${index + 1}" loading="lazy">`;
...
// Initialize gallery navigation with scroll
function initGalleryNavigation() {
    ...
    // Enable mouse drag for gallery scrolling
    let isDown = false;
    let startX;
    let scrollLeft;
